...
public class MssChecker implements IChecker {
    ...
    public MssChecker(HttpResponse expectedResponse) {
    ...
    public boolean check(HttpResponse response) {
        if(response.getStatus() == expectedResponse.getStatus()) {
            if((response.getData() != null) && (!response.getData().isEmpty())) {
                return true;
            } else if((expectedResponse.getData().equals("null")) && (response.getData() == null)) {
                return true;
            } else if((expectedResponse.getData().isEmpty()) && (response.getData().isEmpty())) {
                return true;
            } else {
                return false;
            }
    ...
    public RestfulResponse doSend(HttpRequest request) throws ServiceException {
